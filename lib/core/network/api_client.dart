...
/// Same shape as guru's ApiClient (extracted from siswa's previously monolithic
...
class ApiClient {
  ...
  static Future<dynamic> get(String endpoint) async {
    try {
      ...
      final response = await http.get(Uri.parse("${ApiConfig.baseUrl}$endpoint"), headers: h);
  ...
  static Future<dynamic> delete(String endpoint) async {
    try {
      final h = await _buildHeaders();
      final response = await http.delete(Uri.parse("${ApiConfig.baseUrl}$endpoint"), headers: h);
      return _handleResponse(response);
    } catch (e) {
      return {
        'success': false,
